...
$(function() {
	...
	$('a[href^="#target-"], [data-href^="#target-"]').on('click', function (e) {
		...
		$('html, body').animate({
			scrollTop: ($('*').filter('[data-id="' + thisAttrHref + '"]').length) ? $('*').filter('[data-id="' + thisAttrHref + '"]').offset().top - 50 : $(thisAttrHref).offset().top - 50
		}, 750);
	});
	...
	mainHelloList.on('click', function () {
		...
		mainHelloList.not(_this).find('img').each(function () {
			var _this = $(this);
	...
	if ($('*').is('.about-type-for-slider')) {
		...
		aboutTypeNavSlider.each(function (key, value) {
			$(value).attr('data-index', key).find(aboutTypeNavItem).each(function (key, value) {
				$(value).attr('data-index', key);
			});
		});
		...
		aboutTypeForSlider.on('beforeChange', function(event, slick, currentSlide, nextSlide) {
			var _this = $(this);
			var thisIndex = parseInt(_this.attr('data-index'), 10);
			var aboutTypeNavPos = aboutTypeNavSlider.filter('[data-index="' + thisIndex + '"]').find('.item').filter('[data-index="' + nextSlide + '"]').parent().position().left;
		...
		projectNavMenuLi.each(function () {
			...
			if (_this.hasClass('is-active')) {
				setTimeout(function () {
					...
					_this.parents('.project-nav__scroll').stop().animate({
	...
	searchRClear.on('click', function () {
		var _this = $(this);
		_this.parent().removeClass('is-clear').find(searchRInput).val('');
	});
    ...
    showAllBtns.forEach(function(btn) {
        let open = false;
        const btnText = btn.querySelector('.show-all__text');
        btn.addEventListener('click', function(event) {
            event.preventDefault();
            btn.classList.toggle('active');
            open = !open;
            btnText.textContent = open ? 'Скрыть' : 'Смотреть все'
        })
    })
